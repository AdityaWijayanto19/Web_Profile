...
@push('styles')
<style>
    :root { --m-green: #2c974b; --m-border: rgba(255, 255, 255, 0.1); }

    /* 1. Layout & Typography */
    .ce-block__content, .ce-toolbar__content { max-width: 720px; }
    .ce-paragraph { font-size: 1.2rem; line-height: 1.8; color: #e5e5e5; }
    .title-input {
        background: transparent; border: none; outline: none;
        font-size: 3.5rem; font-weight: 800; width: 100%;
        color: white; margin-bottom: 10px; letter-spacing: -0.04em;
    }
    ...
    .image-tool--filled { background: transparent !important; }

    /* Caption Style */
    .image-tool__caption {
        border: none !important; text-align: center !important;
        font-size: 0.9rem !important; color: rgba(255,255,255,0.4) !important;
        background: transparent !important; font-style: italic;
    }

    /* 3. General UI */
    #editorjs.drag-over { background: rgba(44, 151, 75, 0.03); outline: 2px dashed var(--m-green); }
    .ce-toolbar__plus, .ce-toolbar__settings-btn { color: white !important; background: rgba(255,255,255,0.05) !important; }
    .ce-popover, .ce-inline-toolbar { background: #1a151c !important; border: 1px solid var(--m-border) !important; }
</style>
@endpush
...
<div class="max-w-4xl mx-auto py-16 px-6 pb-40">
    ...
    <div class="fixed bottom-10 right-10 z-50">
        <button id="publishBtn" class="bg-[#2c974b] hover:bg-[#237a3d] text-white px-8 py-2.5 rounded-full font-bold shadow-2xl flex items-center gap-2 transition-all">
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const uploader = (file) => new Promise((res) => {
            ...
            reader.onload = (e) => res({ success: 1, file: { url: e.target.result } });
        ...
        const editor = new EditorJS({
            ...
            tools: {
                ...
                image: {
                    class: ImageTool,
                    config: { uploader: { uploadByFile: uploader } },
                    inlineToolbar: true
                }
            },
            onReady: () => {
        ...
        function bindEvents() {
            ...
            editorContainer.addEventListener('click', (e) => {
                ...
                if (imageToolElem) {
                    ...
                    if (imgBlock) {
                        ...
                        editorContainer.querySelectorAll('[contenteditable="true"]').forEach(el => el.blur());
                    ...
                    focusedBlocks.forEach(block => {
                        ...
                        if (block.querySelector('.image-tool')) {
                            block.classList.remove('ce-block--focused');
                            console.log('✓ Removed ce-block--focused from image block (clicked elsewhere)');
                        }
                    });
                }
            });
                    ...
                    if (focusedBlock && focusedBlock.querySelector('.image-tool')) {
                        ...
                        if (editor.blocks && editor.blocks.delete && blockIndex >= 0) {
                            try {
                                ...
                                setTimeout(async () => {
                                    await saveEditorState();
                                    console.log('✓ Image block deleted & state saved to undo stack');
                                }, 50);
                            } catch(err) {
                ...
                if (e.key === 'ArrowUp' || e.key === 'ArrowDown') {
                    ...
                    if (e.key === 'ArrowUp') {
                        ...
                        if (index === 0 || index === -1) {
                            ...
                            console.log('✓ ArrowUp on first block - skip image selection (go to title)');
                            return;
                        }
                    }
                    ...
                    setTimeout(() => {
                        ...
                        if (currentIndex >= 0 && allBlocks[currentIndex]) {
                            ...
                            allBlocks.forEach(block => block.classList.remove('ce-block--focused'));
                            ...
                            if (currentBlock.querySelector('.image-tool')) {
                                ...
                                // Blur any text elements to remove cursor
                                editorContainer.querySelectorAll('[contenteditable="true"]').forEach(el => el.blur());
                                console.log('✓ Arrow nav - Image selected at index:', currentIndex);
                            } else {
                                // If current block is text, re-focus it to show cursor
                                const textBlock = currentBlock.querySelector('[contenteditable="true"]');
                                if (textBlock) {
                                    textBlock.focus();
            ...
            ['dragleave', 'drop'].forEach(evt => {
                editorContainer.addEventListener(evt, () => editorContainer.classList.remove('drag-over'));
            });
        ...
        document.getElementById('publishBtn').onclick = async () => {
            ...
            console.log("FINAL DATA:", { title: titleInput.value, content: output });
