<aside
    ...
    <div class="p-4 flex items-center gap-2 mb-2 shrink-0 overflow-hidden">
        <div class="p-1.5 rounded-sm shrink-0">
            <img class="w-7 h-7" src="{{ asset('assets/images/pie.svg') }}" alt="pie">
        </div>
    ...
    <nav class="flex-1 px-2 space-y-1 overflow-y-auto overflow-visible custom-scrollbar">
        ...
        @foreach ($navItems as $item)
            <a href="{{ isset($item['route']) ? route($item['route']) : $item['url'] }}"
                class="nav-item group relative flex items-center gap-3 px-3 py-2.5 rounded-sm transition-all hover:bg-white/5 hover:text-white {{ Request::is($item['pattern']) ? 'sidebar-active' : '' }}"
                data-tooltip-name="{{ $item['name'] }}"
                data-tooltip-desc="{{ $item['desc'] }}"
                data-tooltip-icon="{{ $item['icon'] }}">
