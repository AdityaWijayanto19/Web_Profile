    ...
    @if($hero)
        <main class="relative flex-1 flex flex-col md:flex-row items-center z-10 w-full max-w-7xl mx-auto px-6 md:px-16 py-12">
            ...
            <div class="w-full md:w-3/5 space-y-12 z-10 text-left pt-20 md:pt-0">
                <div class="space-y-6 pt-14">
                    <h1 class="text-6xl md:text-[100px] font-bold tracking-tighter leading-[0.85] flex flex-col italic">
                        <span class="text-white not-italic">{{ strtoupper($hero->nama_depan) }} P.</span>
                        <span class="text-primary drop-shadow-[0_0_40px_rgba(244,63,94,0.25)]">{{ strtoupper($hero->nama_belakang) }}</span>
                    </h1>
                ...
                <div class="pl-2">
                    @if($hero->link_cv)
                        <a href="{{ $hero->link_cv }}" target="_blank" rel="noopener noreferrer" class="w-full md:w-auto px-12 py-5 bg-white text-black text-[11px] font-black uppercase tracking-[0.3em] hover:bg-primary hover:text-white transition-all duration-500 rounded-sm inline-flex items-center justify-center">
                            Get in Touch
                        </a>
                    @else
            ...
            <div class="absolute right-0 top-16 h-full w-full md:w-[45%] pointer-events-none z-20 flex items-end justify-end opacity-30 md:opacity-100">
                <img src="{{ $hero->path_foto ? asset('storage/' . $hero->path_foto) : asset('assets/images/me.png') }}" alt="Profile"
        ...
        <main class="relative flex-1 flex flex-col md:flex-row items-center z-10 w-full max-w-7xl mx-auto px-6 md:px-16 py-12">
            ...
            <div class="w-full md:w-3/5 space-y-12 z-10 text-left pt-20 md:pt-0">
                <div class="p-8 bg-[#1a151d]/50 border border-white/10 rounded-sm text-center">
                    <i data-lucide="alert-circle" class="w-12 h-12 text-gray-600 mx-auto mb-3"></i>
                    <p class="text-sm text-gray-400 mb-4">Hero section belum dikonfigurasi</p>
                    <p class="text-xs text-gray-500">Silakan atur di admin dashboard untuk menampilkan profil Anda</p>
                </div>
            </div>
        </main>
    @endif
