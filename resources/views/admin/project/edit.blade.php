...
<div class="max-w-[1200px]">
    ...
    <div class="mb-6 flex justify-between items-center">
        <a href="{{ route('projects.index') }}" class="inline-flex items-center gap-2 text-xs text-gray-500 hover:text-[#730c1e] transition-colors group">
            <i data-lucide="arrow-left" class="w-4 h-4 group-hover:-translate-x-1 transition-transform"></i>
            BACK TO PROJECTS
        </a>
        <span class="text-[9px] text-[#730c1e] font-bold border border-[#730c1e]/30 px-2 py-0.5 rounded-sm uppercase tracking-tighter">Editing Entry</span>
    </div>

    <form action="{{ route('projects.update', ['project' => $proyek->id ?? 1]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <div class="lg:col-span-8 space-y-6">
                ...
                <div class="bg-[#1a151d] border border-white/5 rounded-sm overflow-hidden shadow-2xl">
                    ...
                    <div class="p-6 space-y-6">
                        <!-- Project Title -->
                        <div class="space-y-2">
                            <label class="text-[10px] font-bold text-gray-500 uppercase tracking-[0.1em]">Project Title</label>
                        ...
                        <!-- Project Description -->
                        <div class="space-y-2">
                            <label class="text-[10px] font-bold text-gray-500 uppercase tracking-[0.1em]">Description</label>
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            <!-- Demo Link -->
                            <div class="space-y-2">
                                <label class="text-[10px] font-bold text-gray-500 uppercase tracking-[0.1em]">Demo Link</label>
                            ...
                            <!-- Repository Link -->
                            <div class="space-y-2">
                                <label class="text-[10px] font-bold text-gray-500 uppercase tracking-[0.1em]">Repository Link</label>
                        ...
                        <!-- Order Number -->
                        <div class="space-y-2">
                            <label class="text-[10px] font-bold text-gray-500 uppercase tracking-[0.1em]">Display Order</label>
                ...
                <div class="flex items-center justify-between p-6 bg-[#1a151d] border border-white/5 rounded-sm shadow-xl">
                    <a href="{{ route('projects.index') }}" class="text-xs font-bold text-gray-500 hover:text-white transition-colors uppercase tracking-widest">
                        Cancel
                    </a>
            ...
            <div class="lg:col-span-4 space-y-6">
                ...
                <div class="bg-[#1a151d] border border-white/5 rounded-sm p-6 shadow-xl">
                    <h4 class="text-white text-[11px] font-bold uppercase tracking-widest mb-4 pb-3 border-b border-white/5 flex items-center gap-2">
                    ...
                    <div class="relative aspect-video rounded-sm overflow-hidden border border-white/5 bg-black mb-4">
                        <div class="absolute inset-0 bg-gradient-to-b from-black/20 to-black/80 flex items-center justify-center">
                            @if($proyek->path_gambar)
                                <img id="preview-image" src="{{ asset('storage/' . $proyek->path_gambar) }}" alt="Preview" class="w-full h-full object-cover">
                            @else
                        ...
                        <!-- Upload Overlay -->
                        <label class="absolute inset-0 flex flex-col items-center justify-center bg-black/50 opacity-0 hover:opacity-100 transition-opacity cursor-pointer">
                            ...
                            <input type="file" name="gambar" id="image-input" class="hidden" accept="image/*" @error('gambar') is-invalid @enderror>
                        </label>
                    </div>
                ...
                <div class="bg-[#1a151d] border border-white/5 rounded-sm p-6 shadow-xl">
                    ...
                    <div class="space-y-4">
                        <!-- Status Selection -->
                        <div class="space-y-3">
                            <label class="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Publish Status</label>
                            <div class="grid grid-cols-2 gap-2">
                                <label class="cursor-pointer group">
                                    <input type="radio" name="status" value="draft" class="peer sr-only" {{ old('status', $proyek->status) === 'draft' ? 'checked' : '' }} />
                                    <div class="text-center py-2.5 text-[10px] border border-white/5 bg-black/40 text-gray-500 peer-checked:border-[#730c1e] peer-checked:text-white peer-checked:bg-[#730c1e]/10 transition-all rounded-sm uppercase font-black">DRAFT</div>
                                </label>
                                <label class="cursor-pointer group">
                                    <input type="radio" name="status" value="published" class="peer sr-only" {{ old('status', $proyek->status) === 'published' ? 'checked' : '' }} />
                                    <div class="text-center py-2.5 text-[10px] border border-white/5 bg-black/40 text-gray-500 peer-checked:border-[#730c1e] peer-checked:text-white peer-checked:bg-[#730c1e]/10 transition-all rounded-sm uppercase font-black">PUBLISHED</div>
                                </label>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-[#1a151d] border border-white/5 rounded-sm p-6 shadow-xl">
                    ...
                    <div class="space-y-2 max-h-64 overflow-y-auto custom-scrollbar">
                        @forelse($teknologis ?? [] as $tech)
                            <label class="flex items-center gap-3 p-2.5 rounded-sm bg-black/20 hover:bg-black/40 transition-colors cursor-pointer">
                ...
                <div class="bg-[#1a151d] border border-white/5 rounded-sm p-6">
                    ...
                    <ul class="space-y-4">
                        <li class="flex gap-3">
                            ...
                            <p class="text-[11px] text-gray-400 leading-relaxed">Ubah gambar untuk mengganti yang lama. Gambar lama akan dihapus otomatis.</p>
                        </li>
                        <li class="flex gap-3">
                            <div class="w-5 h-5 rounded bg-[#730c1e]/10 border border-[#730c1e]/20 flex items-center justify-center flex-shrink-0">
                                <span class="text-[10px] text-[#730c1e] font-bold">2</span>
                            </div>
                            <p class="text-[11px] text-gray-400 leading-relaxed">Setidaknya satu teknologi harus dipilih untuk project.</p>
                        </li>
                        <li class="flex gap-3">
                            <div class="w-5 h-5 rounded bg-[#730c1e]/10 border border-[#730c1e]/20 flex items-center justify-center flex-shrink-0">
                                <span class="text-[10px] text-[#730c1e] font-bold">3</span>
                            </div>
                            <p class="text-[11px] text-gray-400 leading-relaxed">Simpan perubahan untuk memperbarui project di portfolio.</p>
                        </li>
                    </ul>
                </div>
            </div>
