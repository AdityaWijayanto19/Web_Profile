...
@section('title', 'Create New Project - Pie')
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <textarea name="deskripsi" rows="6" placeholder="Describe your project, features, and outcomes..."
                                class="w-full bg-black/40 border border-white/10 rounded-sm px-4 py-3 text-white outline-none focus:border-[#730c1e] transition-all placeholder:text-gray-800 resize-none text-sm leading-relaxed @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi') }}</textarea>
                            @error('deskripsi')
                                <p class="text-xs text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                <label class="cursor-pointer group">
                                    <input type="radio" name="status" value="draft" class="peer sr-only" {{ old('status', 'draft') === 'draft' ? 'checked' : '' }} />
                                ...
                                <label class="cursor-pointer group">
                                    <input type="radio" name="status" value="published" class="peer sr-only" {{ old('status') === 'published' ? 'checked' : '' }} />
                        ...
                        @forelse($teknologis ?? [] as $tech)
                            <label class="flex items-center gap-3 p-2.5 rounded-sm bg-black/20 hover:bg-black/40 transition-colors cursor-pointer">
                                <input type="checkbox" name="teknologis[]" value="{{ $tech->id }}"
                                    ...
                                    {{ in_array($tech->id, old('teknologis', [])) ? 'checked' : '' }}>
