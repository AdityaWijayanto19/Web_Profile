        ...
        <div class="bg-[#1a151d] border border-white/5 rounded-sm p-6">
            ...
            <form action="{{ route('admin.footer.update', $footer->id) }}" method="POST" enctype="multipart/form-data"
                ...
                <div>
                    ...
                    <input type="text" id="nama_web" name="nama_web" value="{{ old('nama_web', $footer->nama_web) }}"
                ...
                <div>
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="3"
                        class="w-full px-3 py-2 bg-[#0a0808] border border-white/10 rounded-xs text-white text-sm placeholder-gray-600 focus:outline-none focus:border-[#730c1e] transition-colors"
                        placeholder="Website description..." required>{{ old('deskripsi', $footer->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="no_hp" name="no_hp" value="{{ old('no_hp', $footer->no_hp) }}"
                        class="w-full px-3 py-2 bg-[#0a0808] border border-white/10 rounded-xs text-white text-sm placeholder-gray-600 focus:outline-none focus:border-[#730c1e] transition-colors"
                ...
                <div>
                    ...
                    @if ($footer->logo_path)
                        <div class="mb-2">
                            <img src="{{ asset('storage/' . $footer->logo_path) }}" alt="Current logo"
                ...
                <div class="border-t border-white/5 pt-4">
                    ...
                    <div id="media-container" class="space-y-2">
                        @forelse($footer->mediaSozials as $index => $media)
                                ...
                                <select name="media_sozials[{{ $index }}][technology_id]"
                                    ...
                                    @foreach ($technologies as $tech)
                                        <option value="{{ $tech->id }}"
                                            {{ $tech->id == $media->technology_id ? 'selected' : '' }}>
                                            {{ $tech->nama }}
                                        </option>
                                    @endforeach
                                </select>
                                <input type="url" name="media_sozials[{{ $index }}][url]"
                                    value="{{ $media->url }}" placeholder="https://..."
                ...
                <div>
                    ...
                    <input type="number" id="urutan" name="urutan" value="{{ old('urutan', $footer->urutan) }}"
                        min="0"
                        class="w-full px-3 py-2 bg-[#0a0808] border border-white/10 rounded-xs text-white text-sm placeholder-gray-600 focus:outline-none focus:border-[#730c1e] transition-colors">
                    @error('urutan')
                        <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
