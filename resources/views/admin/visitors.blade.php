...
@section('content')
    <div class="max-w-7xl mx-auto px-4">
            ...
            <div class="stat-card rounded-sm p-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="stat-value text-2xl mt-2">{{ $totalVisitors }}</p>
                    </div>
            ...
            <div class="stat-card rounded-sm p-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="stat-value text-2xl mt-2">{{ $uniqueVisitors }}</p>
                    </div>
                    <div class="text-[#730c1e] opacity-20">
            ...
            <div class="stat-card rounded-sm p-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="stat-value text-2xl mt-2">{{ $pageViews }}</p>
                    </div>
                    <div class="text-[#730c1e] opacity-20">
                ...
                <tbody>
                    @forelse($visitors as $index => $visitor)
                        <tr class="group border-b border-white/5 transition-colors duration-200 hover:bg-white/[0.02]">
                            <td class="px-4 py-4 text-center text-gray-500">{{ ($visitors->currentPage() - 1) * 5 + $loop->iteration }}</td>
                            <td class="px-4 py-4">
                                <code class="ip-address text-[#730c1e] bg-black/50 px-2 py-1 rounded">{{ $visitor->ip_address }}</code>
                            </td>
                            <td class="px-4 py-4">
                                <span class="text-white font-medium text-xs">{{ $visitor->page_path ?? '/' }}</span>
                            </td>
                            <td class="px-4 py-4">
                                <p class="text-gray-400 text-xs line-clamp-1 max-w-xs">{{ Str::limit($visitor->user_agent ?? 'Unknown', 50) }}</p>
                            </td>
                            <td class="px-4 py-4">
                                <span class="text-gray-500 text-xs">
                                    {{ $visitor->visited_at ? $visitor->visited_at->format('M d, Y H:i') : '-' }}
                                </span>
                            </td>
                        </tr>
                    @empty
        ...
        <div class="mt-4">
            {{ $visitors->links('partials.pagination') }}
        </div>
    </div>
@endsection
